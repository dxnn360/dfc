<div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
    <div class="overflow-x-auto">
        <table class="w-full">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach($suratPengantar as $i => $item)
                <tr class="hover:bg-gray-50 transition-colors duration-150">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div class="flex items-center gap-2">
                            <!-- Detail Button -->
                            <a href="{{ route('supervisor.surat-pengantar.detail', $item->id) }}" 
                            ...
                            <!-- ACC Button -->
                            <form action="{{ route('supervisor.surat-pengantar.approve', $item->id) }}" method="POST" class="inline">
                                @csrf
                                <button type="submit" 
                            ...
                            <div x-data="{ open: false }" class="relative">
                                ...
                                <div x-show="open" 
                                    ...
                                    <div class="bg-white rounded-xl shadow-lg max-w-md w-full p-6" @click.away="open = false">
                                        <h3 class="text-lg font-semibold text-gray-900 mb-4">Berikan Catatan Revisi</h3>
                                        <form action="{{ route('supervisor.surat-pengantar.reject', $item->id) }}" method="POST">
                                            @csrf
                                            <textarea name="catatan_supervisor" 
                                                      placeholder="Masukkan catatan revisi untuk surat pengantar..."
    ...
    <!-- Empty State -->
    @if($suratPengantar->count() === 0)
    <div class="text-center py-12">
        <svg class="w-16 h-16 mx-auto text-gray-400 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
        </svg>
        <p class="text-gray-500 text-lg">Tidak ada data surat pengantar</p>
    </div>
    @endif
</div>
