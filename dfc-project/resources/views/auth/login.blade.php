<x-guest-layout>
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-black">Silahkan Login menggunakan email dan password yang telah diberikan</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-5">
        ...
        <div class="flex items-center justify-between pt-1">
            <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                <input id="remember_me" 
                    ...
                    name="remember">
                <span class="ml-2 text-sm text-neutral-600 group-hover:text-neutral-900 transition">{{ __('Ingat Saya') }}</span>
            </label>
        ...
        <!-- Login Button -->
        <div class="pt-2">
            <x-primary-button class="w-full justify-center py-3 bg-blue-400 hover:bg-blue-800 focus:bg-blue-800 active:bg-blue-950 text-white font-medium rounded-xl transition-all duration-200 shadow-sm hover:shadow-md">
                {{ __('Sign In') }}
            </x-primary-button>
        </div>
    </form>
